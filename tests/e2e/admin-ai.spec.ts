...
test.describe('AI Module', () => {
  test.beforeEach(async ({ page }) => {
    await navigateAsAdmin(page, '/admin/ai');
  });

  test('should navigate to ai page', async ({ page }) => {
    await expect(page.locator('aside')).toBeVisible();
    await expect(page.locator('main')).toBeVisible();
    await takeScreenshot(page, 'ai-page');
  });

  test('should show page content when data loads @requires-api', async ({ page }) => {
    const main = page.locator('main');
    const hasContent = await main.locator('text=/AI|Inteligência/').isVisible({ timeout: 5000 }).catch(() => false);
    if (hasContent) {
      await takeScreenshot(page, 'ai-content');
    }
  });
  ...
  test('should render table when data loads @requires-api', async ({ page }) => {
    ...
    if (hasTable) {
      ...
      await takeScreenshot(page, 'ai-table');
    }
  });
