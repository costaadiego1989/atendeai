...
test.describe('Inventory Module', () => {
  test.beforeEach(async ({ page }) => {
    await navigateAsAdmin(page, '/admin/inventory');
  });
  ...
  test('should show page content when data loads @requires-api', async ({ page }) => {
    ...
    const hasContent = await main.locator('text=/Inventory|Estoque/').isVisible({ timeout: 5000 }).catch(() => false);
    if (hasContent) {
      await takeScreenshot(page, 'inventory-content');
    }
  });
  ...
  test('should render table when data loads @requires-api', async ({ page }) => {
    ...
    if (hasTable) {
      ...
      await takeScreenshot(page, 'inventory-table');
    }
  });
