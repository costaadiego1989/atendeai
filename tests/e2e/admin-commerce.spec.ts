...
test.describe('Commerce Module', () => {
  test.beforeEach(async ({ page }) => {
    await navigateAsAdmin(page, '/admin/commerce');
  });

  test('should navigate to commerce page', async ({ page }) => {
    await expect(page.locator('aside')).toBeVisible();
    await expect(page.locator('main')).toBeVisible();
    await takeScreenshot(page, 'commerce-page');
  });

  test('should show page content when data loads @requires-api', async ({ page }) => {
    const main = page.locator('main');
    const hasContent = await main.locator('text=/Commerce|Comércio/').isVisible({ timeout: 5000 }).catch(() => false);
    if (hasContent) {
      await takeScreenshot(page, 'commerce-content');
    }
  });
  ...
  test('should render table when data loads @requires-api', async ({ page }) => {
    ...
    if (hasTable) {
      ...
      await takeScreenshot(page, 'commerce-table');
    }
  });
