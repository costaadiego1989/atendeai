...
import {
  CheckCircle2, Loader2, ArrowRight, ArrowLeft, Sparkles,
  ShoppingCart, Stethoscope, GraduationCap, Building2,
  MessageSquareMore, Users, Brain, Zap, Crown, Globe,
  TrendingUp, Clock, HeadphonesIcon, BarChart3,
  Rocket, Puzzle, Package, Store, Briefcase, Heart,
  Utensils, Wrench, Palette, Cog
} from "lucide-react";
...
const PAIN_COPY_MAP: Record<string, { label: string; description: string }> = {
  triagem_ruim: {
    label: "Triagem ruim",
    description: "Leads chegam sem prioridade e o time perde tempo com contatos frios.",
  },
  linguagem_sensivel: {
    label: "Linguagem sensível",
    description: "Equipe precisa de padrão de comunicação com mais segurança e consistência.",
  },
  agenda_desorganizada: {
    label: "Agenda desorganizada",
    description: "Conflitos de horários e baixa previsibilidade no atendimento.",
  },
};

const ICON_MAP: Record<string, React.ElementType> = {
  ShoppingCart, Stethoscope, GraduationCap, Building2,
  Store, Briefcase, Heart, Utensils, Wrench, Palette,
  Rocket, Cog, Globe, MessageSquareMore, Users, Brain,
  Zap, Crown, TrendingUp, Clock, HeadphonesIcon, Package,
  DEFAULT: Briefcase,
};
...
const VOLUME_OPTIONS: VolumeOption[] = [
  { id: "low", label: "Até 300 conversas/mês", description: "Negócio em fase inicial ou operação enxuta", icon: Zap, recommendation: "ESSENCIAL" },
  { id: "mid", label: "300 a 500 conversas/mês", description: "Operação estabelecida com crescimento constante", icon: Crown, recommendation: "PROFISSIONAL" },
  { id: "high", label: "Mais de 500 conversas/mês", description: "Alta demanda exigindo escala e performance", icon: Globe, recommendation: "ESCALA" },
];
...
const PricingSection: React.FC<PricingSectionProps> = ({ onSignupClick, hideHeader = false }) => {
  ...
  const selectNiche = (niche: NicheAPI) => {
    ...
    setSelectedPains([]); // reset pains when niche changes
  ...
  useEffect(() => {
    ...
    const augmentedPlans = useMemo(() => {
      return plansData?.plans?.map((plan) => ({
        ...plan,
        totalMonthlyPrice: plan.monthlyPrice + (nicheModules.reduce((acc, m) => acc + (m.monthlyPrice || 0), 0)),
      }));
    }, [plansData, nicheModules]);

    const recommendedPlanData = augmentedPlans?.find((p) => p.code === recommendedPlan);
    const RecommendedIcon = PLAN_ICONS[recommendedPlan] || Sparkles;

    const painIcons = [Clock, Brain, Users, TrendingUp, MessageSquareMore, HeadphonesIcon, Rocket, Package];
    const TOTAL_STEPS = 5;

    const getNicheIcon = (iconName: string): React.ElementType => {
        ...
        <div className="max-w-3xl mx-auto relative z-10">
          {!hideHeader && (
            <div className="text-center mb-16">
              ...
              >
                Descubra o plano <span className="text-primary">ideal para você</span>
              </motion.h2>
            </div>
          )}
                ...
                <AnimatePresence mode="wait" custom={direction}>
                  {step === 0 && (
                    ...
                    >
                      <StepHeader
                        ...
                        subtitle="Selecione o setor mais próximo da sua operação para uma recomendação personalizada."
                      />
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {nichesData?.niches?.map((niche) => {
                          const NicheIcon = getNicheIcon(niche.iconName);
                  ...
                  {step === 1 && selectedNiche && (
                    ...
                    >
                      <StepHeader
                        step={2}
                        title="Quais são suas maiores dores?"
                        subtitle={`Dores comuns em ${selectedNiche.displayName}. Selecione todas as que já acontecem hoje.`}
                      />
                  ...
                  {step === 2 && (
                    ...
                    >
                      <StepHeader
                        step={3}
                        title="Qual o volume do seu atendimento?"
                        subtitle="Isso determina a potência ideal da infraestrutura para sua operação."
                      />
                  ...
                  {step === 3 && (
                    ...
                    >
                      ...
                      <h3 className="text-xl font-black text-white mb-3">Analisando seu perfil...</h3>
                      <p className="text-sm text-white/40 text-center max-w-sm">
                        Cruzando dados do segmento {selectedNiche?.displayName}, volume de operação e {selectedPains.length} dor{selectedPains.length !== 1 ? "es" : ""} selecionada{selectedPains.length !== 1 ? "s" : ""}.
                      </p>
                    ...
                    >
                      <div className="text-center mb-8">
                        ...
                        >
                          ...
                          Plano Recomendado para {selectedNiche?.displayName}
                        </motion.div>
                      </div>
                        ...
                        <div className="relative z-10">
                          <div className="flex items-start justify-between mb-8">
                            ...
                            <div className="text-right">
                              ...
                              <span className="block text-sm text-white/40 font-semibold">/mês</span>
                            </div>
                          </div>

                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-8">
                             <div className="p-4 rounded-xl bg-white/[0.04] border border-white/5">
                                <span className="text-[10px] font-black text-primary/60 uppercase tracking-tighter mb-1 block underline underline-offset-4 decoration-primary/30">Mensagens</span>
                                <span className="text-lg font-black text-white">{recommendedPlanData.messagesQuota.toLocaleString('pt-BR')}</span>
                             </div>
                             <div className="p-4 rounded-xl bg-white/[0.04] border border-white/5">
                                <span className="text-[10px] font-black text-primary/60 uppercase tracking-tighter mb-1 block underline underline-offset-4 decoration-primary/30">Tokens IA</span>
                                <span className="text-lg font-black text-white">{recommendedPlanData.aiTokensQuota.toLocaleString('pt-BR')}</span>
                             </div>
                             <div className="p-4 rounded-xl bg-white/[0.04] border border-white/5">
                                <span className="text-[10px] font-black text-primary/60 uppercase tracking-tighter mb-1 block underline underline-offset-4 decoration-primary/30">CRM Contatos</span>
                          ...
                          {nicheModules.length > 0 && (
                            ...
                            >
                              <h4 className="text-xs font-black uppercase tracking-widest text-primary/80 mb-4 flex items-center gap-2">
                                ...
                                Módulos extras para {selectedNiche?.displayName}
                              </h4>
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                {nicheModules.map((mod, i) => (
                                  ...
                                  >
                                    ...
                                    {mod.monthlyPrice === 0 && (
                                      <span className="text-[10px] font-black text-primary/60 px-2 py-0.5 rounded-full bg-primary/10 whitespace-nowrap">
                                        INCLUSO
                                      </span>
                                    )}
                                  </motion.div>
                          ...
                          <motion.button
                            whileHover={{ scale: 1.02 }}
                            whileTap={{ scale: 0.98 }}
                            onClick={() => onSignupClick(recommendedPlanData.code)}
                              ...
                              {augmentedPlans.map((plan) => {
                                ...
                                return (
                                  ...
                                  >
                                    <div className="flex items-center gap-3 mb-4">
                                      ...
                                      <div className="min-w-0">
                                        <h4 className="text-sm font-black text-white flex items-center gap-2 truncate">
                                          {plan.displayName}
                                          {isRecommended && (
                                            <span className="text-[9px] px-2 py-0.5 rounded-full bg-primary/20 text-primary font-black">IDEAL</span>
                                          )}
                                        </h4>
                                    ...
                                    <button
                                      onClick={() => onSignupClick(plan.code)}
                                      className={cn(
