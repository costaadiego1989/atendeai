...
@Injectable()
export class CompleteMetaInstagramConnectionUseCase {
  constructor(
  ...
  async execute(input: Input): Promise<{
    ...
    if (payload.branchId) {
      const branch = (await this.tenantRepository.listBranches(payload.tenantId)).find(
        (item) => item.id.toValue() === payload.branchId,
      );
      if (!branch) {
