...
export interface VerifyTwilioWhatsAppSenderInput {
  tenantId: string;
      ...
      const sender = await this.twilioManagementAcl.verifySender(
        {
          ...
          wabaId,
          account: this.resolveTwilioAccount(override.credentials),
        },
      ...
      await this.tenantRepository.updateBranch(branch.id.toValue(), {
        ...
        whatsAppConfigOverride: {
          ...
          credentials: {
            ...
            wabaId: sender.configuration?.wabaId || wabaId,
    ...
    const sender = await this.twilioManagementAcl.verifySender(
      {
        ...
        verificationCode: input.verificationCode,
        wabaId,
        account: this.resolveTwilioAccount(tenant.whatsAppConfig.credentials),
      },
      { tenantId: input.tenantId },
    );
