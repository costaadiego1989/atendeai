...
export interface RecoveryAsyncJobView {
  id: string;
  tenantId: string;
...
export class RecoveryAsyncJobsService {
  ...
  async createJob(input: {
    ...
    const rows = await this.prisma.$queryRaw<RecoveryAsyncJobRow[]>(Prisma.sql`
      INSERT INTO recovery_schema.recovery_async_jobs (
        tenant_id,
        branch_id,
        type,
        status,
  ...
  async getJob(tenantId: string, jobId: string): Promise<RecoveryAsyncJobView> {
    await this.ensureTableShape();
    const rows = await this.prisma.$queryRaw<RecoveryAsyncJobRow[]>(Prisma.sql`
      SELECT *
      FROM recovery_schema.recovery_async_jobs
    ...
    if (!rows[0]) {
      throw new EntityNotFoundException('RecoveryAsyncJob', jobId);
    }
  ...
  async getDownloadPayload(
    ...
    const rows = await this.prisma.$queryRaw<RecoveryAsyncJobRow[]>(Prisma.sql`
      ...
      WHERE tenant_id = ${tenantId}::uuid
        AND id = ${jobId}::uuid
        AND type = 'EXPORT_RECOVERY_REPORT_CSV'
    ...
    return {
      fileName: rows[0].file_name ?? `relatorio-cobranças-${jobId}.csv`,
  ...
  private toView(row: RecoveryAsyncJobRow): RecoveryAsyncJobView {
    return {
      id: row.id,
