...
export class PrismaWidgetSessionRepository implements IWidgetSessionRepository {
  ...
  async findById(id: string, tenantId: string): Promise<WidgetSessionData | null> {
    const raw = await this.prisma.widgetSession.findFirst({
      where: { id, tenantId },
    });
    return raw ? this.map(raw) : null;
  ...
  async update(id: string, tenantId: string, data: UpdateWidgetSessionData): Promise<WidgetSessionData> {
    ...
    const existing = await this.prisma.widgetSession.findUnique({ where: { id } });
