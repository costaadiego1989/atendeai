...
@Injectable()
export class DispatchProspectExecutionUseCase
  implements IDispatchProspectExecutionUseCase
{
  constructor(
  ...
  async execute(
    ...
    const renderedMessage = this.renderTemplate(messageTemplate, contact.name);
    const dispatchResult = await this.messagingFacade.queueSystemMessage({
