...
export class GenerateProposalPdfUseCase {
  ...
  private async generatePdfBuffer(proposal: any): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      ...
      doc.moveDown(0.5);
      doc.fontSize(12).fillColor('black').text(proposal.description || '');
      doc.moveDown();

      if (proposal.benefits) {
        doc.fontSize(14).fillColor('#27ae60').text('Diferenciais e Benefícios:');
      ...
      proposal.items.forEach((item: any) => {
        ...
        doc.text(`R$ ${(item.unitPrice * item.quantity).toFixed(2)}`, 500, currentY);
        currentY += 20;
      });
      ...
      doc.fontSize(16).fillColor('#2c3e50').text(`VALOR TOTAL: R$ ${proposal.totalAmount.toFixed(2)}`, { align: 'right' });
