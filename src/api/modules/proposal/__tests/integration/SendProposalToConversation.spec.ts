...
import { buildProposal, createMessagingFacadeMock, InMemoryProposalRepository } from '../../__tests__/proposal-test-utils';
...
describe('SendProposalToConversationService', () => {
  it('creates a public proposal link and queues the message in the conversation', async () => {
    ...
    const proposalPublicLinkService = new ProposalPublicLinkService(repository as any, {
      get: (key: string) => {
    ...
    const sendProposalToConversationService = new SendProposalToConversationService(
      repository as any,
