...
export class AsaasAdapter implements IPaymentGateway {
    ...
    constructor(private readonly configService: ConfigService) {
        ...
        this.httpClient = axios.create({
            baseURL: baseUrl || (isSandbox ? 'https://sandbox.asaas.com/api/v3' : 'https://www.asaas.com/api/v3'),
            headers: {
                access_token: apiKey || '',
    ...
    async deletePayment(paymentId: string): Promise<PaymentLinkStatusResult> {
        try {
            ...
            return {
                id: response.data.id,
                status: response.data.deleted ? 'REMOVED' : response.data.status ?? 'REMOVED',
            };
        } catch (error: any) {
    ...
    async restorePayment(paymentId: string): Promise<PaymentLinkStatusResult> {
        try {
            const response = await this.httpClient.post(`/payments/${paymentId}/restore`);
            return {
                id: response.data.id,
    ...
    async removePaymentLink(
        ...
        try {
            const response = await this.httpClient.delete(`/paymentLinks/${paymentLinkId}`);
            return {
                id: response.data.id,
                status: response.data.deleted ? 'REMOVED' : response.data.status ?? 'REMOVED',
            };
    ...
    async restorePaymentLink(
        ...
        try {
            const response = await this.httpClient.post(`/paymentLinks/${paymentLinkId}/restore`);
            return {
                id: response.data.id,
                status: response.data.status ?? 'ACTIVE',
            };
    ...
    private extractTenantId(rawReference?: string): string | undefined {
        ...
        const billingUpgradeMatch = /^billing-upgrade\|([^|]+)\|(ESSENCIAL|PROFISSIONAL|ESCALA)$/.exec(
            rawReference,
        );
        ...
        const salesChargeMatch = /^sales-charge\|([^|]+)\|([^|]+)$/.exec(rawReference);
        if (salesChargeMatch) {
