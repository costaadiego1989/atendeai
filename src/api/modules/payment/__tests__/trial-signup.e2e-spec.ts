...
describe('Trial Signup Flow (e2e)', () => {
    ...
    async function cleanupAllTestData() {
        ...
        for (const tenant of tenants) {
            ...
            await (prisma as any).whatsappConfig?.deleteMany({ where: { tenantId } }).catch(() => { });
            await (prisma as any).aiConfig?.deleteMany({ where: { tenantId } }).catch(() => { });
            await (prisma as any).tenantAgentRule?.deleteMany({ where: { tenantId } }).catch(() => { });
            await (prisma as any).tenantFinancialAccount?.deleteMany({ where: { tenantId } }).catch(() => { });
    ...
    it('should block login when trial is expired', async () => {
        ...
        expect(loginResponse.body.error.message).toContain('Seu período de teste (7 dias) expirou');
    });
