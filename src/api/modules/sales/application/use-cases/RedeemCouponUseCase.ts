...
export class RedeemCouponUseCase {
  ...
  async execute(input: { tenantId: string; couponId?: string; code?: string }) {
    ...
    const now = new Date();
    if (now < coupon.startsAt) throw new BadRequestException('Coupon has not started yet');
    if (coupon.expiresAt && now > coupon.expiresAt) throw new BadRequestException('Coupon has expired');
    if (coupon.maxUses > 0 && coupon.usedCount >= coupon.maxUses) {
      throw new BadRequestException('Coupon usage limit reached');
    }

    const updated = await this.repo.incrementCouponUsage(input.tenantId, coupon.id);
    return {
      coupon: updated,
      discount: {
