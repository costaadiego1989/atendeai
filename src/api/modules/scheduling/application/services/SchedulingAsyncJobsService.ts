...
export interface SchedulingAsyncJobView {
  id: string;
  tenantId: string;
...
@Injectable()
export class SchedulingAsyncJobsService {
  ...
  async getJob(tenantId: string, jobId: string): Promise<SchedulingAsyncJobView> {
    ...
    if (!rows[0]) {
      throw new EntityNotFoundException('SchedulingAsyncJob', jobId);
    }
  ...
  async getDownloadPayload(
    ...
    const rows = await this.prisma.$queryRaw<SchedulingAsyncJobRow[]>(Prisma.sql`
      ...
      WHERE tenant_id = ${tenantId}::uuid
        AND id = ${jobId}::uuid
        AND type = 'EXPORT_SCHEDULING_REPORT_CSV'
    ...
    return {
      fileName: rows[0].file_name ?? `relatorio-agenda-${jobId}.csv`,
  ...
  private toView(row: SchedulingAsyncJobRow): SchedulingAsyncJobView {
    return {
      id: row.id,
