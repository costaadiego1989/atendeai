...
export class BillingTenantHandlers implements OnModuleInit {
  ...
  onModuleInit() {
    this.eventBus.subscribe('tenant.created', async (event) => {
      ...
      const planDefinition = await this.billingRepository.findPlanByCode(payload.plan);
      ...
      if (
        ...
        (payload.plan === 'ESSENCIAL' || existingSubscription.asaasSubscriptionId)
      ) {
        return;
      }
      ...
      const subscription =
        ...
        Subscription.create(
          ...
          payload.plan,
          planDefinition
            ? buildSubscriptionCommercialState(planDefinition, existingModules)
            : undefined,
        );
    ...
    this.eventBus.subscribe('tenant.plan-changed', async (event) => {
      ...
      const existingModules = await this.billingRepository.listSubscriptionModules(
    ...
    this.eventBus.subscribe('payment.trial-subscription-initiated.v1', async (event) => {
      const payload = event.payload as unknown as TrialInitiatedPayload;
      const tenantId = payload.tenantId;

      let subscription = await this.billingRepository.findSubscription(tenantId);
      const plan = payload.plan as PlanType;
      const planDefinition = await this.billingRepository.findPlanByCode(plan);
      ...
      subscription.updateAsaasInfo(payload.asaasCustomerId, payload.asaasSubscriptionId);
      subscription.activate();
      
      await this.billingRepository.saveSubscription(subscription);

      let usage = await this.billingRepository.getUsage(tenantId, subscription.billingCycleStart);
      if (!usage) {
