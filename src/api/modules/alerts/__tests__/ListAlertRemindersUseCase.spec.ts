...
describe('ListAlertRemindersUseCase', () => {
  ...
  it('returns all reminders for a tenant/user', async () => {
    ...
    expect(reminderRepository.findAllByUser).toHaveBeenCalledWith('tenant-1', 'user-1', undefined);
  });

  it('filters by branchId when provided', async () => {
    reminderRepository.findAllByUser.mockResolvedValue([
      { id: 'r1', tenantId: 'tenant-1', userId: 'user-1', branchId: 'branch-1' },
    ]);
    ...
    expect(reminderRepository.findAllByUser).toHaveBeenCalledWith('tenant-1', 'user-1', 'branch-1');
  });
