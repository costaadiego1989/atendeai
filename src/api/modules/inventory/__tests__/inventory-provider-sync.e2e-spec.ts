...
describe('InventoryProviderSync (e2e)', () => {
  ...
  async function triggerSync(connectionId: string): Promise<{ jobId: string }> {
    const res = await request(app.getHttpServer())
      .post(
        `/api/v1/tenants/${tenantId}/inventory/connections/${connectionId}/sync`,
      )
      .set('Cookie', [authCookie])
      .expect(202);
  ...
  async function waitForJobCompletion(
    jobId: string,
  ): Promise<{ status: InventoryAsyncJobStatus }> {
  ...
  beforeAll(async () => {
    ...
    const tenant = await prisma.tenant.create({
      data: {
        companyName: 'Provider Sync Store',
        cnpj: tenantCnpj,
        plan: 'ESSENCIAL',
      },
    });
  ...
  afterAll(async () => {
    ...
    if (tenantId) {
      await prisma
        .$executeRaw(
          Prisma.sql`DELETE FROM inventory_schema.inventory_async_jobs WHERE tenant_id = ${tenantId}::uuid`,
        )
        .catch(() => {});
      await prisma
        .$executeRaw(
  ...
  it('INV-T-090a: sync via Bling persiste itens no banco com sku, qty e status corretos', async () => {
      ...
      .mockResolvedValueOnce({
        ...
        json: async () => ({
          data: [
            {
              id: 501,
              codigo: 'BLING-E2E-001',
              nome: 'Produto Bling E2E',
              estoque: { saldoVirtual: 15 },
              preço: 49.9,
            },
            {
              id: 502,
              codigo: 'BLING-E2E-002',
              nome: 'Produto Zerado',
    ...
    const conn = await createConnection('BLING', 'Bling', {
      accessToken: 'e2e-bling-tok',
    });
        ...
        json: async () => [
          {
            ...
            body: {
              id: 'MLB111',
              title: 'Produto ML E2E',
              available_quantity: 5,
              price: 99.9,
              currency_id: 'BRL',
              seller_custom_field: 'ML-E2E-001',
            },
          },
          {
            code: 200,
            body: {
              id: 'MLB222',
              title: 'Produto ML E2E 2',
              available_quantity: 0,
              price: 49.9,
              currency_id: 'BRL',
              seller_custom_field: 'ML-E2E-002',
            },
          },
        ],
      }) as unknown as typeof fetch;
  ...
  it('INV-T-090e: após sync bem-sucedido a conexão tem lastSyncedAt não nulo', async () => {
    ...
    const conn = await createConnection('BLING', 'Bling LastSync', {
      accessToken: 'e2e-tok-lastsync',
    });
  ...
  it('INV-T-090f: erro HTTP 500 do provider marca job como FAILED e lastSyncedAt não é atualizado', async () => {
    ...
    const conn = await createConnection('BLING', 'Bling Error Test', {
      accessToken: 'e2e-tok-err',
    });
    const { jobId } = await triggerSync(conn.id);
    const job = await waitForJobCompletion(jobId);
  ...
  it('INV-T-090g: item com SKU vazio é ignorado e demais itens são persistidos', async () => {
          ...
          data: [
            {
              ...
              codigo: '',
              nome: 'Sem SKU',
              estoque: { saldoVirtual: 1 },
              preço: 10,
            },
            {
              id: 802,
              codigo: 'VALID-SKU-001',
              nome: 'Válido',
              estoque: { saldoVirtual: 3 },
              preço: 20,
            },
          ],
        }),
    ...
    const conn = await createConnection('BLING', 'Bling Partial', {
      accessToken: 'e2e-tok-partial',
    });
    const { jobId } = await triggerSync(conn.id);
    await waitForJobCompletion(jobId);
