...
describe('WooCommerceProvider', () => {
  ...
  it('INV-T-065a: testConnection lança erro quando storeUrl, consumerKey ou consumerSecret ausente', async () => {
    await expect(
      provider.testConnection({ consumerKey: 'k', consumerSecret: 's' }),
    ).rejects.toThrow();
    await expect(
    ...
    await expect(
      provider.testConnection({ storeUrl: 'https://x.com', consumerKey: 'k' }),
    ).rejects.toThrow();
  });
  ...
  it('INV-T-065c: fetchStock chama GET /wp-json/wc/v3/products com per_page e page', async () => {
    const fetchMock = jest
      .fn()
      .mockResolvedValue(makeWooPage([])) as unknown as typeof fetch;
    global.fetch = fetchMock;
