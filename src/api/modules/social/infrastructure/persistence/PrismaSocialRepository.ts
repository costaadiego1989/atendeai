...
export class PrismaSocialRepository implements ISocialRepository {
    ...
    await prisma.$executeRawUnsafe(
      `INSERT INTO social_schema.social_accounts
       ...
       ON CONFLICT (tenant_id, platform, external_account_id) DO UPDATE SET
         ...
         updated_at = NOW()`,
      account.id.toValue(), account.tenantId, account.platform,
      account.externalAccountId, account.username, account.displayName,
      account.profilePictureUrl, account.accessToken, account.refreshToken,
      account.tokenExpiresAt, account.pageId, account.webhookSecret,
      account.status, account.connectedAt,
    );
  }

  async findAccountById(tenantId: string, id: string): Promise<SocialAccount | null> {
    const rows = await prisma.$queryRawUnsafe<any[]>(
      `SELECT * FROM social_schema.social_accounts WHERE id = $1 AND tenant_id = $2`, id, tenantId,
    );
    return rows[0] ? this.mapAccount(rows[0]) : null;
  }

  async findAccountByPlatform(tenantId: string, platform: string, externalAccountId: string): Promise<SocialAccount | null> {
    const rows = await prisma.$queryRawUnsafe<any[]>(
  ...
  async listAccounts(tenantId: string): Promise<SocialAccount[]> {
    const rows = await prisma.$queryRawUnsafe<any[]>(
      `SELECT * FROM social_schema.social_accounts WHERE tenant_id = $1 ORDER BY connected_at DESC`, tenantId,
    );
  ...
  async deleteAccount(tenantId: string, id: string): Promise<void> {
    await prisma.$executeRawUnsafe(
      `DELETE FROM social_schema.social_accounts WHERE id = $1 AND tenant_id = $2`, id, tenantId,
    );
  }

  async upsertPost(tenantId: string, post: {
    socialAccountId: string; platform: string; externalPostId: string;
    postType?: string; caption?: string; mediaUrl?: string; permalink?: string; postedAt?: Date;
  }): Promise<string> {
    const rows = await prisma.$queryRawUnsafe<any[]>(
      `INSERT INTO social_schema.social_posts
       ...
       RETURNING id`,
      tenantId, post.socialAccountId, post.platform, post.externalPostId,
      post.postType || null, post.caption || null, post.mediaUrl || null,
      post.permalink || null, post.postedAt || null,
    );
    return rows[0]?.id;
  }
  ...
  async saveComment(comment: SocialComment): Promise<void> {
    await prisma.$executeRawUnsafe(
      ...
      comment.id.toValue(), comment.tenantId, comment.socialAccountId,
      comment.postId, comment.platform, comment.externalCommentId,
      comment.parentCommentId, comment.authorExternalId, comment.authorUsername,
      comment.authorName, comment.text, comment.sentiment, comment.status,
      comment.isHidden, comment.receivedAt, comment.repliedAt,
    );
  }

  async findCommentById(tenantId: string, id: string): Promise<SocialComment | null> {
    const rows = await prisma.$queryRawUnsafe<any[]>(
      `SELECT * FROM social_schema.social_comments WHERE id = $1 AND tenant_id = $2`, id, tenantId,
    );
    return rows[0] ? this.mapComment(rows[0]) : null;
  }

  async findCommentByExternalId(tenantId: string, externalCommentId: string): Promise<SocialComment | null> {
    const rows = await prisma.$queryRawUnsafe<any[]>(
      `SELECT * FROM social_schema.social_comments WHERE tenant_id = $1 AND external_comment_id = $2`,
      tenantId, externalCommentId,
    );
    return rows[0] ? this.mapComment(rows[0]) : null;
  }

  async listComments(tenantId: string, filters: {
    status?: string; postId?: string; platform?: string; page?: number; limit?: number;
    ...
    if (filters.postId) { conditions.push(`post_id = $${idx}`); params.push(filters.postId); idx++; }
    if (filters.platform) { conditions.push(`platform = $${idx}`); params.push(filters.platform); idx++; }
  ...
  async updateCommentStatus(tenantId: string, commentId: string, status: string, repliedAt?: Date): Promise<void> {
    if (repliedAt) {
      await prisma.$executeRawUnsafe(
        `UPDATE social_schema.social_comments SET status = $1, replied_at = $2 WHERE id = $3 AND tenant_id = $4`,
        status, repliedAt, commentId, tenantId,
      );
    } else {
      await prisma.$executeRawUnsafe(
        `UPDATE social_schema.social_comments SET status = $1 WHERE id = $2 AND tenant_id = $3`,
        status, commentId, tenantId,
      );
    }
  }

  async saveReply(tenantId: string, reply: {
    commentId: string; externalReplyId?: string; text: string;
    repliedBy: 'AI' | 'HUMAN'; ruleId?: string; userId?: string;
    ...
    const rows = await prisma.$queryRawUnsafe<any[]>(
      `INSERT INTO social_schema.social_comment_replies
       ...
       VALUES (gen_random_uuid(), $1, $2, $3, $4, $5, $6, $7, $8, $9, NOW())
       RETURNING id`,
      tenantId, reply.commentId, reply.externalReplyId || null,
      reply.text, reply.repliedBy, reply.ruleId || null,
      reply.userId || null, reply.status, reply.errorMessage || null,
    );
    return rows[0]?.id;
  }

  async listReplies(tenantId: string, commentId: string): Promise<Array<{
    id: string; text: string; repliedBy: string; ruleId?: string; status: string; createdAt: Date;
  }>> {
    const rows = await prisma.$queryRawUnsafe<any[]>(
    ...
    await prisma.$executeRawUnsafe(
      `INSERT INTO social_schema.social_auto_reply_rules
       ...
       ON CONFLICT (id) DO UPDATE SET
         ...
         updated_at = NOW()`,
      rule.id.toValue(), rule.tenantId, rule.name,
      rule.isActive, rule.priority, rule.platform,
      JSON.stringify(rule.conditions), JSON.stringify(rule.actions),
      JSON.stringify(rule.limits), rule.totalFired, rule.lastFiredAt,
    );
  }

  async findRuleById(tenantId: string, id: string): Promise<SocialAutoReplyRule | null> {
    const rows = await prisma.$queryRawUnsafe<any[]>(
      `SELECT * FROM social_schema.social_auto_reply_rules WHERE id = $1 AND tenant_id = $2`, id, tenantId,
    );
    return rows[0] ? this.mapRule(rows[0]) : null;
  }

  async listActiveRules(tenantId: string, platform?: string): Promise<SocialAutoReplyRule[]> {
  ...
  async deleteRule(tenantId: string, id: string): Promise<void> {
    await prisma.$executeRawUnsafe(
      `DELETE FROM social_schema.social_auto_reply_rules WHERE id = $1 AND tenant_id = $2`, id, tenantId,
    );
  }
  ...
  async countRepliesByRuleInLastHour(tenantId: string, ruleId: string): Promise<number> {
    const rows = await prisma.$queryRawUnsafe<any[]>(
  ...
  async countRepliesForPostByRule(tenantId: string, ruleId: string, postId: string): Promise<number> {
    const rows = await prisma.$queryRawUnsafe<any[]>(
      ...
      tenantId, ruleId, postId,
    );
    return rows[0]?.count || 0;
  }

  async findLastReplyToUser(tenantId: string, ruleId: string, authorExternalId: string): Promise<Date | null> {
    const rows = await prisma.$queryRawUnsafe<any[]>(
      ...
      tenantId, ruleId, authorExternalId,
    );
    return rows[0]?.created_at || null;
  }

  async upsertInboxThread(tenantId: string, thread: {
    socialAccountId: string; platform: string; recipientExternalId: string;
    recipientUsername?: string; originCommentId?: string; lastMessageText: string;
  }): Promise<string> {
    const rows = await prisma.$queryRawUnsafe<any[]>(
      `INSERT INTO social_schema.social_inbox_threads
       ...
       ON CONFLICT (tenant_id, platform, recipient_external_id) DO UPDATE SET
         ...
         updated_at = NOW()
       RETURNING id`,
      tenantId, thread.socialAccountId, thread.platform,
      thread.recipientExternalId, thread.recipientUsername || null,
      thread.originCommentId || null, thread.lastMessageText,
    );
    return rows[0]?.id;
  }

  async logAudit(tenantId: string, entry: {
    event: string; entityId?: string; entityType?: string;
    platform?: string; ruleId?: string; metadata?: Record<string, unknown>;
  }): Promise<void> {
    await prisma.$executeRawUnsafe(
      ...
      tenantId, entry.event, entry.entityId || null, entry.entityType || null,
      entry.platform || null, entry.ruleId || null,
      JSON.stringify(entry.metadata || {}),
    );
  }

  async getStats(tenantId: string): Promise<{
    totalComments: number; pendingComments: number; repliedComments: number;
    autoRepliedComments: number; activeRules: number; connectedAccounts: number;
  }> {
